...
do s=1,n_shots
    ...
    write(*,*) nstreamer_head
    write(*,*) nstreamer_tail
    write(*,*) nfx
    write(*,*) "Shot:", s
    !LOOP DO TEMPO
    do k=1, passos
    ...
    write(shot_char,"(i3)") s
    call save_seismogram
...
write(*,*) "Tempo de processamento decorrido:",nint((final_time-start_time)/60),"minutos."
...
allocate (image(Nz,Nx),final_image(Nz,Nx))
...
!#######################################################
subroutine save_seismogram()
implicit none

open(25,file="seismogram"//trim(adjustl(shot_char))//".bin",status="replace",access="direct",form="unformatted",recl=Nx*k)
write(25,rec=1) seismogram 
close(25)

end subroutine save_seismogram

!#######################################################
subroutine save_image()
implicit none

open (37,file="image"//trim(adjustl(shot_char))//".bin",status="replace",access="direct",form="unformatted", &
& recl=Nx*Nz)
...
open (39,file="final_image.bin",status="replace",access="direct",form="unformatted", &
& recl=Nx*Nz)
write(39,rec=1) final_image
close(39)

write(*,*) "Imagem final empilhada."
